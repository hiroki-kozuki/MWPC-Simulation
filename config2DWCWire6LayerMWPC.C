...
int main(int argc, char * argv[]) {
  ...
  for (int i = 0; i < 100; ++i) {                // 100 wires.
    ...
    cathodeWires1[i] = new SolidWire(0.0, ypos, 0., cathodeRadius, cathodeHalflength, 1, 0, 0); // Last 3 inputs define the axis of wires.
  ...
  for (int i = 0; i < 100; ++i) {
    ...
    cathodeWires2[i] = new SolidWire(0.0, ypos, 1., cathodeRadius, cathodeHalflength, 1, 0, 0);
  ...
  for (int i = 0; i < 100; ++i) {
    ...
    cathodeWires3[i] = new SolidWire(xpos, 0.0, 1.5, cathodeRadius, cathodeHalflength, 0, 1, 0);
    cathodeWires3[i]->SetBoundaryPotential(-4000.);
    cathodeWires3[i]->SetLabel("cathode3");
    geo.AddSolid(cathodeWires3[i], &W);
  }
  // Generate 100 wires for 4th cathode layer (repeating in x).
  std::vector<SolidWire*> cathodeWires4(100, nullptr);
  for (int i = 0; i < 100; ++i) {
    int j = i - 50;
    double xpos = 0.05 + (double)j*0.1;
    cathodeWires4[i] = new SolidWire(xpos, 0.0, 2.5, cathodeRadius, cathodeHalflength, 0, 1, 0);
    cathodeWires4[i]->SetBoundaryPotential(-4000.);
    cathodeWires4[i]->SetLabel("cathode4");
    geo.AddSolid(cathodeWires4[i], &W);
  }
