...
int main(int argc, char * argv[]) {
  ...
  const double cog_x3 = 0., cog_y3 = 0., cog_z3 = 1.5;  // 5mm appart from anode plane.
  // Coordinates of centre of gravity of film 4 [cm]:
  const double cog_x4 = 0., cog_y4 = 0., cog_z4 = 2.5;  // 5mm appart from anode plane.

  // Half lengths of film [cm]:
  const double hl_x = 5., hl_y = 5., hl_z = 0.00125; // 25 microns thick
  SolidBox cathode1(cog_x1, cog_y1, cog_z1, hl_x, hl_y, hl_z);
  cathode1.SetBoundaryPotential(-4000.);
  // cathode1.SetLabel(const std::string& label);
  SolidBox cathode2(cog_x2, cog_y2, cog_z2, hl_x, hl_y, hl_z);
  cathode2.SetBoundaryPotential(-4000.);
  // cathode2.SetLabel(const std::string& label);
  SolidBox cathode3(cog_x3, cog_y3, cog_z3, hl_x, hl_y, hl_z);
  cathode3.SetBoundaryPotential(-4000.);
  // cathode3.SetLabel(const std::string& label);
  SolidBox cathode4(cog_x4, cog_y4, cog_z4, hl_x, hl_y, hl_z);
  cathode4.SetBoundaryPotential(-4000.);
  // cathode4.SetLabel(const std::string& label);

  geo.AddSolid(&cathode1, &W);
  geo.AddSolid(&cathode2, &W);
  geo.AddSolid(&cathode3, &W);
  geo.AddSolid(&cathode4, &W);
  ...
  for (int i = 0; i < 100; ++i) {                
    ...
    anodeWiresY[i] = new SolidWire(0.0, ypos, 2.0, anodeRadius, anodeHalflength, 1, 0, 0); 
  ...
  for (const auto& cluster : track.GetClusters()) {
		...
		drift.DriftElectron(electron.x, electron.y, electron.z, electron.t);	// Causes the sharp peaks in the singal.
  ...
  potViewZ.SetArea(-5., -5., 0., 5., 5., 2.5);
  ...
  potViewX.SetArea(-5., -5., 0., 5., 5., 2.5);
  potViewX.SetPlane(0., -1., 0., 0., 0., 1.25); // Plot at (0,0,1.25) with +ve y axis into the page.
  potViewX.PlotContour("v");
  // Weighting potential around anodeX.
  //ViewField wpotViewX(&nebem);
  //wpotViewX.SetArea(-5., -5., 0., 5., 5., 2.5);
  //wpotViewX.SetPlane(0., -1., 0., 0., 0., 1.25); // Plot at (0,0,1.25) with +ve y axis into the page.
  //wpotViewX.PlotContourWeightingField("anodeX","v");
  // Actual potential in y-z plane.
  ViewField potViewY(&nebem);
  potViewY.SetArea(-5., -5., 0., 5., 5., 2.5);
  potViewY.SetPlane(-1., 0., 0., 0., 0., 1.25); // Plot at (0,0,1.25) with +ve x axis into the page.
  potViewY.PlotContour("v");
  // Weighting potential around anodeY.
  //ViewField wpotViewY(&nebem);
  //wpotViewY.SetArea(-5., -5., 0., 5., 5., 2.5);
  //wpotViewY.SetPlane(-1., 0., 0., 0., 0., 1.25); // Plot at (0,0,1.25) with +ve x axis into the page.
  //wpotViewY.PlotContourWeightingField("anodeY","v");

  // Plot field and weighting field profiles along z.
  ViewField fieldView(&nebem);
  fieldView.PlotProfile(0., 0., 0., 0., 0., 2.5);
  // Actual field in x-y plane.
  ViewField fieldViewZ(&nebem);
  fieldViewZ.SetArea(-5., -5., 0., 5., 5., 2.5);
  ...
  fieldViewX.SetArea(-5., -5., 0., 5., 5., 2.5);
  fieldViewX.SetPlane(0., -1., 0., 0., 0., 1.25);  // Plot at (0,0,1.25) with +ve y axis into the page.
  fieldViewX.PlotContour("e");
  // Weighting field around anodeX.
  //ViewField wfieldViewX(&nebem);
  //wfieldViewX.SetArea(-5., -5., 0., 5., 5., 2.5);
  //wfieldViewX.SetPlane(0., -1., 0., 0., 0., 1.25);  // Plot at (0,0,1.25) with +ve y axis into the page.
  ...
  fieldViewY.SetArea(-5., -5., 0., 5., 5., 2.5);
  fieldViewY.SetPlane(-1., 0., 0., 0., 0., 2.0);  // Plot at (0,0,2.0) with +ve x axis out of the page.
  fieldViewY.PlotContour("e");
  // Weighting field around anodeY.
  //ViewField wfieldViewY(&nebem);
  //wfieldViewY.SetArea(-5., -5., 0., 5., 5., 2.5);
  //wfieldViewY.SetPlane(-1., 0., 0., 0., 0., 2.0);  // Plot at (0,0,2.0) with +ve x axis out of the page.
